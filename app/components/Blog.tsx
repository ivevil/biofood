...
interface BlogProps {
    block: {
        ...
        component: string,
        description: string,
        details: Array<{ title: string; description: string; image: string; }>
        id: string,
        image: string,
        sectionName: string,
        title: string
            ...
            <div className="blog bg-center bg-no-repeat bg-[url('/vegetables-twisted.jpg')] bg-blend-multiply bg-cover py-8" style={{ margin: "0 auto" }} id="blog">
                ...
                <div className="w-3/4 flex flex-end flex-wrap justify-end">
                    {props.block.details.map((detail, index) => {
                        return (
                            <BlogCard detailImage={detail.image} detailTitle={detail.title} detailDescription={detail.description} key={index} />
                        )
                    })}
                    </div>
                </div>
